...
declare global {
  ...
  /** Team CSS = ['none', 'res', 'enl', 'mac'] */
  const TEAM_TO_CSS: [string, string, string, string];
  /** Team codes = ['N', 'R', 'E', 'M'] */
  const TEAM_CODES: [string, string, string, string];

  /** ['Neutral', 'Resistance', 'Enlightened', 'U̶͚̓̍N̴̖̈K̠͔̍͑̂͜N̞̥͋̀̉Ȯ̶̹͕̀W̶̢͚͑̚͝Ṉ̨̟̒̅'] */
  const TEAM_NAMES: [string, string, string, string];
